...
| 2 | Типы и протоколы | ⏳ В работе | - |
| 3 | Database connection pool | 📋 Запланировано | - |
| 4 | Рефакторинг handlers (DI) | 📋 Запланировано | - |
| 5 | Обновление main.py и graceful shutdown | 📋 Запланировано | - |
| 6 | Тесты для Database | 📋 Запланировано | - |
...
#### Задачи
- [ ] Исправить `src/storage/models.py`: `Message.role: str` → `Literal["user", "assistant"]`
- [ ] Создать `src/storage/protocols.py` с `DatabaseProtocol`
- [ ] Создать `src/llm/protocols.py` с `LLMClientProtocol`
- [ ] Добавить импорт Protocol в `src/storage/__init__.py`
- [ ] Добавить импорт Protocol в `src/llm/__init__.py`

#### Проверки
- [ ] `make format` - код отформатирован
- [ ] `make lint` - нет ошибок линтера
- [ ] `make typecheck` - mypy проходит без ошибок типизации
- [ ] Код соответствует `conventions.mdc` (type hints обязательны)
- [ ] Процесс соответствует `workflow.mdc` (обновлен tasklist)
...
#### Задачи
- [ ] Добавить атрибут `self._connection: aiosqlite.Connection | None = None` в `Database.__init__`
- [ ] Реализовать `async def connect(self) -> None` - открытие соединения
- [ ] Реализовать `async def close(self) -> None` - закрытие соединения
- [ ] Реализовать `async def __aenter__(self)` и `async def __aexit__(self, ...)` для context manager
- [ ] Переделать `save_message()` на использование `self._connection`
- [ ] Переделать `get_history()` на использование `self._connection`
- [ ] Переделать `clear_history()` на использование `self._connection`
- [ ] Добавить проверку `if not self._connection: raise RuntimeError("Database not connected")`

#### Проверки
- [ ] `make format` и `make lint` - чисто
- [ ] `make typecheck` - типы корректны
- [ ] Бот запускается и работает с новой версией Database
- [ ] Код соответствует `conventions.mdc` (async-first, fail fast)
- [ ] Процесс соответствует `workflow.mdc` (тест пройден)
...
#### Задачи
- [ ] Удалить глобальные переменные из `handlers.py` (строки 16-19: llm_client, database, config)
- [ ] Удалить функцию `setup_handlers()` (строки 22-34)
- [ ] Создать класс `BotHandlers` с конструктором принимающим (llm_client, database, config)
- [ ] Создать `self.router = Router()` внутри класса
- [ ] Переделать декоратор `@router.message(Command("start"))` на метод класса
- [ ] Переделать декоратор `@router.message()` на метод класса
- [ ] Добавить метод `_register_handlers()` для регистрации handlers
- [ ] Обновить `src/bot/bot.py` для работы с `BotHandlers.router`

#### Проверки
- [ ] `make format`, `make lint`, `make typecheck` - всё зелёное
- [ ] Бот работает с новой архитектурой
- [ ] Код соответствует `conventions.mdc` (1 класс = 1 файл, нет глобального state)
- [ ] Процесс соответствует `workflow.mdc` (согласование, тест)
...
#### Задачи
- [ ] Обновить `main.py`: использовать `async with database:` вместо `await database.init_db()`
- [ ] Создать экземпляр `BotHandlers` с зависимостями
- [ ] Передать `bot_handlers.router` в диспетчер
- [ ] Удалить вызов `setup_handlers()`
- [ ] Обновить `create_bot()` для работы с внешним router
- [ ] Добавить graceful shutdown для Database (закрытие при Ctrl+C)

#### Проверки
- [ ] `make format`, `make lint`, `make typecheck` - чисто
- [ ] Бот запускается и корректно завершается (Ctrl+C)
- [ ] Соединение с БД закрывается при завершении
- [ ] Код соответствует `conventions.mdc` (async-first)
- [ ] Процесс соответствует `workflow.mdc` (обновлен tasklist)
