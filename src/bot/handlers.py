...
async def handle_message(message: TelegramMessage) -> None:
    ...
    logger.info(
        f"Received message from user_id={user_id}, chat_id={chat_id}, length={len(message.text)}"
    )

    try:
        # 1. Сохраняем сообщение пользователя
        user_message = Message(user_id=user_id, chat_id=chat_id, role="user", content=message.text)
        await database.save_message(user_message)

        # 2. Загружаем историю диалога
        history = await database.get_history(
            chat_id=chat_id, user_id=user_id, limit=config.max_history_messages
        )
        ...
        response = await llm_client.get_response(
            messages=history, system_prompt=config.system_prompt
        )
